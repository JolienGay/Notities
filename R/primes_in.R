...
primes_in <- function(x, y){
  ...
  sequence <- c(x:y)
  # We make an empty list to put the primes in. These are used to evaluate what
  # other numbers are primes as well.
  ...
  for (n in c(2:y)){
    ...
    counter <- 0
    # So here, if a number can be divided by a number in primes_lower_than_Y,
    # it isn't put in primes_lower_than_y, if it can't, it is.
